...
async def run_benchmark(
    ...
    for i in range(num_iterations):
        ...
        claim = Claim(
            text=claim_text, context="Testing cache performance", source="benchmark", confidence=1.0
        )
...
async def compare_performance(iterations: int = 3, unique_claims: bool = False) -> dict[str, Any]:
    ...
    no_cache_results = await run_benchmark(
        num_iterations=iterations, cache_enabled=False, unique_claims=unique_claims
    )

    logger.info("Running benchmark WITH cache...")
    with_cache_results = await run_benchmark(
        num_iterations=iterations, cache_enabled=True, unique_claims=unique_claims
    )
    ...
    improvement = {
        ...
        "percentage_improvement": ((no_cache_mean - with_cache_mean) / no_cache_mean) * 100
        if no_cache_mean > 0
        else 0,
...
async def run_claim_variants(base_claims: list[str], variants_per_claim: int = 3) -> dict[str, Any]:
    ...
    for base_claim in base_claims:
        claim_results = {"base_claim": base_claim, "variants": [], "cache_hits": 0}
        ...
        # Now test variants
        for _i in range(variants_per_claim):
            ...
            # Gather evidence
            await hunter.gather_evidence(variant_obj)
        ...
        claim_results["hit_rate"] = (
            claim_results["cache_hits"] / variants_per_claim if variants_per_claim > 0 else 0
        )
...
async def main():
    ...
    parser.add_argument(
        "--unique", action="store_true", help="Use unique claims for each iteration"
    )
    parser.add_argument("--variants", type=int, default=3, help="Number of variants per claim")
    parser.add_argument(
        "--output", type=str, default="cache_benchmark_results.json", help="Output file"
    )
