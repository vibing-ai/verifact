...
from typing import Dict, List, Any, Optional, Union, Tuple, Generator, TypeVar, Generic
...
from src.models.factcheck import Claim, Evidence, Verdict, VerdictType
...
class SupabaseClient:
    ...
    def __init__(self, min_conn=1, max_conn=10):
        ...
        if not self.openai_api_key and OPENAI_AVAILABLE:
            logger.warning("OpenAI API key not found. Set OPENAI_API_KEY environment variable for embedding functionality.")
    ...
    def _init_connection_pool(self):
        ...
        try:
            ...
            logger.info(f"Created PostgreSQL connection pool (min={self.min_conn}, max={self.max_conn})")
        except Exception as e:
    ...
    def generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            ...
            response = client.embeddings.create(
                ...
                input=text.replace("\n", " ")[:8191]  # Truncate to max token length and replace newlines
            )
    ...
    @retry_on_error()
    def store_embedding(self, content: str, embedding: List[float], metadata: Dict[str, Any] = None) -> Dict[str, Any]:
        """
    ...
    def store_claim_with_embedding(self, claim: Union[Claim, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            with self.get_cursor() as cur:
                ...
                entities_json = json.dumps(claim_dict.get("entities", [])) if claim_dict.get("entities") else '[]'
    ...
    @retry_on_error()
    def store_evidence(self, evidence: Union[Evidence, Dict[str, Any]], claim_id: str) -> Dict[str, Any]:
        """
    ...
    def store_verdict(self, verdict: Union[Verdict, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            with self.get_cursor() as cur:
                ...
                sources_json = json.dumps(verdict_dict.get("sources", [])) if verdict_dict.get("sources") else '[]'
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> Dict[str, Any]:
        ...
        try:
            with self.get_cursor() as cur:
                ...
                # Delete old verdicts
                cur.execute("DELETE FROM verdicts WHERE created_at < %s RETURNING id", (cutoff_date,))
                deleted_verdicts = cur.rowcount
                
                # Delete old evidence
                cur.execute("DELETE FROM evidence WHERE created_at < %s RETURNING id", (cutoff_date,))
                deleted_evidence = cur.rowcount
                
                # Delete old embeddings
                cur.execute("DELETE FROM embeddings WHERE created_at < %s RETURNING id", (cutoff_date,))
    ...
    @retry_on_error()
    def get_feedback_for_claim(self, claim_id: str, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
        """
    ...
    @retry_on_error()
    def _get_recent_comments(self, claim_id: Optional[str] = None, limit: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def _encrypt_sensitive_fields(self, data: Dict[str, Any], sensitive_fields: List[str]) -> Dict[str, Any]:
        """
    ...
    def _decrypt_sensitive_fields(self, data: Dict[str, Any], sensitive_fields: List[str]) -> Dict[str, Any]:
        """
