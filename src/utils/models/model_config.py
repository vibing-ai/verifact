...
# Update imports
from src.utils.logging.logger import get_component_logger, log_performance, performance_timer, request_context
...
# Format: "provider/model" - e.g., "qwen/qwen3-8b:free", "meta-llama/llama-3.3-8b-instruct:free"
DEFAULT_MODELS = {
...
class ModelManager:
    ...
    def _load_parameters(self) -> Dict[str, Any]:
        ...
        # Try to load agent-specific parameters first, fall back to general parameters
        ...
        if not api_key:
            if provider.lower() == "openrouter":
                raise ModelAuthenticationError(
                    f"OpenRouter API key not found. Please set {env_var} environment variable.\n"
                    f"You can get an API key from https://openrouter.ai/keys."
                )
            else:
                raise ModelAuthenticationError(
                    f"API key for {provider} not found. Please set {env_var} environment variable."
                )
    ...
    def get_request_headers(self) -> Dict[str, str]:
        ...
        headers = {
            ...
            "Authorization": f"Bearer {self.get_api_key('openrouter')}"
        }
    ...
    def _generate_cache_key(self, messages: List[Dict[str, str]], parameters: Dict[str, Any]) -> str:
        """
        ...
        cache_dict = {
            "messages": messages,
            "parameters": {k: v for k, v in parameters.items() if k != "stream" and k != "request_timeout"}
        }
    ...
    def _cached_completion(self, cache_key: str, model: str) -> Dict[str, Any]:
        ...
        """
        # This will never be called directly, it's just for the lru_cache decorator
        pass
    ...
    def _handle_error_response(self, response: httpx.Response) -> None:
        ...
        if status_code == 401:
            raise ModelAuthenticationError(f"Authentication error: {error_message}")
            ...
            raise ModelUnavailableError(f"Model not found: {error_message}")
        elif status_code == 408:
            raise ModelTimeoutError(f"Request timed out: {error_message}")
    ...
    @retry(
        retry=retry_if_exception_type((httpx.TimeoutException, ModelTimeoutError, ModelRateLimitError)),
    ...
    async def completion_async(
        ...
        **kwargs
        ...
        with request_context(component=self.agent_type or "general") as request_id:
            # Add request context
            logger.info(
                f"Starting async model completion request",
                extra={
            ...
            if ENABLE_CACHING and not request_parameters.get("stream", False):
                ...
                # Cached values are stored by model, so we need to check each model in the chain
                for model in self.fallback_models:
                    try:
                        # Try to get from cache
                        if hasattr(self._cached_completion, "cache_info"):  # Check if cache is available
                            cache_dict = {
                                "cache_key": cache_key,
                                "model": model
                            }
                            ...
                            if cache_key_str in self._cached_completion.cache:
                                logger.info(f"Cache hit for model {model}", extra={"model": model, "cached": True})
                                return self._cached_completion.cache[cache_key_str]
                    except Exception as e:
                        logger.warning(f"Cache lookup failed: {str(e)}", extra={"error": str(e), "error_type": type(e).__name__})
            ...
            for model in self.fallback_models:
                try:
                    with performance_timer(f"api_call_{model}", logger=logger, level="debug", model=model):
                        async with httpx.AsyncClient(timeout=request_parameters["request_timeout"]) as client:
                            ...
                            logger.debug(f"Sending request to model {model}", extra={"model": model})
                            ...
                            if ENABLE_CACHING and not request_parameters.get("stream", False):
                                ...
                                try:
                                    ...
                                    if hasattr(self._cached_completion, "cache"):
                                        ...
                                        logger.debug(f"Cached response for model {model}", extra={"model": model, "cache_key": cache_key})
                                except Exception as e:
                                    logger.warning(f"Cache storage failed: {str(e)}", extra={"error": str(e), "error_type": type(e).__name__})
                            
                            logger.info(
                                f"Successfully got completion from model {model}",
                                extra={
                                    "model": model,
                                    "status_code": response.status_code,
                                    "usage": response_data.get("usage", {})
                    ...
                    logger.warning(
                        f"Retryable error with model {model}: {str(e)}",
                        extra={
                            "error": str(e),
                            "error_type": type(e).__name__,
                            "model": model,
                    ...
                    # Unexpected errors
                    logger.error(
                        f"Unexpected error with model {model}: {str(e)}",
                        extra={
                            "error": str(e),
                            "error_type": type(e).__name__,
                            "model": model
    ...
    def completion(
        ...
        **kwargs
        ...
        with request_context(component=self.agent_type or "general") as request_id:
            # Add request context
            logger.info(
                f"Starting model completion request",
                extra={
                    "messages_count": len(messages),
                    "parameters": {k: v for k, v in kwargs.items() if k not in ["messages"]}
                }
            ...
            for model in self.fallback_models:
                try:
                    with performance_timer(f"api_call_{model}", logger=logger, level="debug", model=model):
                        with httpx.Client(timeout=request_parameters["request_timeout"]) as client:
                            ...
                            if ENABLE_CACHING and not request_parameters.get("stream", False):
                                cache_key = self._generate_cache_key(messages, request_parameters)
                                try:
                                    # Store in the cache manually since we're not actually calling the function
                                    cache_dict = {
                                        "cache_key": cache_key,
                                        "model": model
                                    }
                    ...
                    logger.warning(
                        f"Rate limit or timeout with model {model}",
                        extra={
                            "error": str(e),
                            "error_type": type(e).__name__,
                            "model": model,
                    ...
                    for attempt in range(MAX_RETRIES):
                        ...
                        logger.info(f"Retrying after {wait_time}s due to {str(e)}", 
                                   extra={"attempt": attempt + 1, "max_retries": MAX_RETRIES, "wait_time": wait_time})
                        time.sleep(wait_time)
                        
                        try:
                            with performance_timer(f"api_call_{model}_retry_{attempt+1}", logger=logger, level="debug", model=model):
                                with httpx.Client(timeout=request_parameters["request_timeout"]) as client:
                                    response = client.post(
                                        OPENROUTER_API_ENDPOINT,
                                        json=payload,
                                    ...
                                    if response.status_code == 200:
                                        response_data = response.json()
                                        self._update_token_usage(response_data)
                                        logger.info(f"Successfully got completion from model {model} after retry",
                            ...
                            logger.warning(f"Retry {attempt+1} failed with {str(retry_error)}", 
                                         extra={"attempt": attempt + 1, "error": str(retry_error)})
                            continue
                        except Exception as retry_error:
                            # Other errors, try next model
                            last_error = retry_error
                            logger.error(f"Unexpected error during retry: {str(retry_error)}", 
    ...
    def create_openai_client(self):
        ...
        try:
            ...
            headers = self.get_request_headers()
            # Filter out content-type and authorization as they're handled by the client
    ...
    def configure_openai_for_agent(self):
        ...
        try:
            ...
            headers = self.get_request_headers()
            # Keep content-type and authorization separate as they're handled by OpenAI client directly
            custom_headers = {k: v for k, v in headers.items() 
                            if k not in ["Content-Type", "Authorization"]}
            ...
            logger.info(f"OpenAI configured to use {self.model_name} via OpenRouter")
    ...
    def _adjust_parameters_for_model(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "deepseek-chat" in model_name:
            # DeepSeek might benefit from slightly higher temperature for reasoning
