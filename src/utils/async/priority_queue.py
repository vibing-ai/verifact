...
from typing import Dict, List, Any, Optional, TypeVar, Generic, Callable, Tuple
...
class PriorityQueue(Generic[T]):
    ...
    def __init__(
        ...
        self._item_map: Dict[str, PrioritizedItem[T]] = {}  # Maps item_id to prioritized item
    ...
    def put(self, item: T, priority: Optional[float] = None, metadata: Optional[Dict[str, Any]] = None) -> str:
        """
        ...
        with self._lock:
            ...
            if not self._allow_duplicate_items:
                # Simple string representation comparison - can be customized for more complex equality
                item_str = str(item)
                if any(str(pi.item) == item_str for pi in self._queue):
                    # Return ID of the existing item instead of adding a duplicate
    ...
    def get_queue_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            avg_pending_time = sum(pending_wait_times) / len(pending_wait_times) if pending_wait_times else 0
...
class ClaimPriorityQueue(PriorityQueue[T]):
    ...
    def __init__(
        ...
        """
        # Define claim priority function (inverting check-worthiness since lower values = higher priority)
        def claim_priority_function(claim: Any) -> float:
            ...
            # Invert check-worthiness so higher check-worthiness = higher priority (lower value)
    ...
    def get_related_claims(self, claim: T) -> List[Tuple[str, T]]:
        ...
        with self._lock:
            for item_id, prioritized_item in self._item_map.items():
                ...
                other_text = str(getattr(other_claim, 'text', '')) if hasattr(other_claim, 'text') else str(other_claim)
                
                # Simple similarity check - this can be enhanced with more sophisticated algorithms
    ...
    def _calculate_similarity(self, text1: str, text2: str) -> float:
        ...
        """
        # Simple word overlap ratio - can be replaced with more sophisticated algorithms
