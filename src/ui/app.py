...
async def main(message: cl.Message):
    ...
    try:
        ...
        with cl.Step(name="Detecting Claims", show_input=True) as step:
            ...
            claims = await claim_detector.detect_claims(
                message.content,
            ...
            check_worthy_claims = [claim for claim in claims if claim.check_worthiness >= claim_detector.min_check_worthiness]
            ...
            if len(check_worthy_claims) > 1:
                ...
                for i, claim in enumerate(check_worthy_claims):
                    claim_checkboxes.append(
                        cl.Checkbox(
                            id=f"claim_{i}",
                            label=f"Claim {i+1}: {claim.text[:60]}..." if len(claim.text) > 60 else claim.text,
                            initial=True
                        )
                    )
                ...
                process_button = cl.Button(
                    id="process_claims_button",
                    label="Process Selected Claims"
                )
                
                # Send the selection UI
                await cl.Message(
                    content=claim_selection_msg,
                    elements=claim_checkboxes
                ).send()
                
                # Send the process button
                await cl.Message(
                    content="",
                    elements=[process_button]
                ).send()
            ...
            await main_msg.update(content=f"Found {len(check_worthy_claims)} check-worthy claims. Gathering evidence...")
        
        # Process the claims
        await process_claims(check_worthy_claims, main_msg, 
                             show_detailed_evidence, show_confidence, 
                             show_feedback_form, detect_related_claims,
                             concurrent_processing, max_concurrent)
...
async def on_feedback(action):
    ...
    if success:
        await cl.Message(content="Thank you for your feedback! It helps us improve VeriFact.").send()
    else:
        await cl.Message(content="There was an error saving your feedback. Please try again later.").send()
...
async def on_view_feedback_admin(action):
    ...
    if not user or getattr(user, "identifier", "") != os.environ.get("VERIFACT_ADMIN_USER", "admin"):
