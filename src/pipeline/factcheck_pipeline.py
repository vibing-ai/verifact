...
class FactcheckPipeline:
    ...
    def _emit_progress(self, stage: PipelineStage, progress: float, message: str, data: Any = None):
        ...
        """
        progress_data = PipelineProgress(
            stage=stage,
            progress=progress,
            message=message,
            data=data
        ...
        while True:
            try:
                ...
                self._emit_progress(
                    PipelineStage.CLAIM_DETECTION, 
                    1.0, 
                    f"Detected {len(claims)} claims",
                    claims
                )
                
                self._emit_event(PipelineEvent.STAGE_COMPLETED, {
                ...
                retry_count += 1
                self.logger.error(
                    f"Error detecting claims (attempt {retry_count}): {str(e)}",
                    exc_info=True
                )
                
                self._emit_event(PipelineEvent.ERROR, {
                    "stage": PipelineStage.CLAIM_DETECTION,
                    "error": str(e),
                    "attempt": retry_count
                })
    ...
    async def _gather_evidence(self, claim: Claim) -> List[Evidence]:
        ...
        while True:
            try:
                ...
                self._emit_event(PipelineEvent.EVIDENCE_GATHERED, {
                    "claim": claim,
                    "evidence": evidence
                })
                ...
                retry_count += 1
                self.logger.error(
                    f"Error gathering evidence (attempt {retry_count}): {str(e)}",
                    exc_info=True
                )
    ...
    async def _generate_verdict(self, claim: Claim, evidence: List[Evidence]) -> Optional[Verdict]:
        ...
        while True:
            try:
                ...
                self._emit_event(PipelineEvent.VERDICT_GENERATED, {
                    "claim": claim,
                    "evidence": evidence,
                    "verdict": verdict
                })
                ...
                retry_count += 1
                self.logger.error(
                    f"Error generating verdict (attempt {retry_count}): {str(e)}",
                    exc_info=True
                )
        ...
        try:
            ...
            for i, claim in enumerate(claims):
                ...
                if not evidence:
                    ...
                    self._emit_event(PipelineEvent.WARNING, {
                        "stage": PipelineStage.EVIDENCE_GATHERING,
            ...
            self.stats["total_processing_time"] = self.stats["completed_at"] - self.stats["started_at"]
    ...
    async def process_text_streaming(self, text: str) -> AsyncIterator[Verdict]:
        ...
        try:
            ...
            self._emit_event(PipelineEvent.COMPLETED, {
                "verdict_count": self.stats["verdicts_generated"]
            })
...
def create_default_pipeline(config: Optional[PipelineConfig] = None) -> FactcheckPipeline:
    ...
    evidence_hunter = EvidenceHunter(
        model_name=config.evidence_hunter_model
    )
    
    verdict_writer = VerdictWriter(
        model_name=config.verdict_writer_model
    )
