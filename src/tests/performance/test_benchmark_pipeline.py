...
class BenchmarkResults:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "claim_detection": {
                ...
                "average": (
                    sum(self.claim_detection_times) / len(self.claim_detection_times)
                    if self.claim_detection_times
                    else 0
                ),
            ...
            "evidence_gathering": {
                ...
                "average": (
                    sum(self.evidence_gathering_times) / len(self.evidence_gathering_times)
                    if self.evidence_gathering_times
                    else 0
                ),
            ...
            "verdict_generation": {
                ...
                "average": (
                    sum(self.verdict_generation_times) / len(self.verdict_generation_times)
                    if self.verdict_generation_times
                    else 0
                ),
            ...
            "pipeline": {
                ...
                "average": (
                    sum(self.pipeline_times) / len(self.pipeline_times)
    ...
    def print_summary(self):
        ...
        print(
            tabulate(
                target_data, headers=["Component", "Actual", "Target", "Status"], tablefmt="grid"
            )
        )
    ...
    def plot_results(self, output_file: Optional[str] = None):
        ...
        axs[0, 0].bar(
            range(len(results["claim_detection"]["times"])), results["claim_detection"]["times"]
        )
        axs[0, 0].axhline(
            y=results["claim_detection"]["average"], color="r", linestyle="-", label="Average"
        )
        ...
        axs[0, 1].bar(
            range(len(results["evidence_gathering"]["times"])),
            results["evidence_gathering"]["times"],
        )
        axs[0, 1].axhline(
            y=results["evidence_gathering"]["average"], color="r", linestyle="-", label="Average"
        )
        ...
        axs[1, 0].bar(
            range(len(results["verdict_generation"]["times"])),
            results["verdict_generation"]["times"],
        )
        axs[1, 0].axhline(
            y=results["verdict_generation"]["average"], color="r", linestyle="-", label="Average"
        )
        ...
        axs[1, 1].axhline(
            y=results["pipeline"]["average"], color="r", linestyle="-", label="Average"
        )
...
class Benchmarker:
    ...
    def __init__(self, args):
        ...
        self.results.set_metadata(
            "command", f"python -m src.tests.test_benchmark_pipeline {' '.join(sys.argv[1:])}"
        )
        self.results.set_metadata(
            "environment",
            {
