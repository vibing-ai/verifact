...
def opinion_text():
    ...
    return "I think chocolate ice cream is the best flavor. In my opinion, dogs are better than cats."
...
class TestClaimDetector:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("The president said taxes will increase, but I don't believe it.", 1),  # Contains opinion and claim
            ("If it rains tomorrow, the game will be canceled.", 0),  # Hypothetical, not a factual claim
        ]
    )
    ...
    async def test_entity_extraction(self, detector, mock_agent, mock_entity_agent):
        ...
        mock_entity_agent.run.return_value = [
            Entity(text="NASA", type="ORGANIZATION", normalized_text="National Aeronautics and Space Administration", relevance=0.8),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Global temperatures have risen by 1°C since pre-industrial times.", 1, ClaimDomain.ENVIRONMENT),
            ("The film won three Academy Awards.", 1, ClaimDomain.ENTERTAINMENT),
        ]
    )
    async def test_claim_detection_by_domain(self, detector, mock_agent, text, expected_claims, domain):
    ...
    async def test_check_worthiness_thresholds(self, detector, mock_agent, check_worthiness, min_threshold, should_be_included):
    ...
    async def test_entity_extraction_by_type(self, detector, mock_agent, mock_entity_agent, text, entity_types):
    ...
    async def test_max_claims_parameter(self, detector, mock_agent, max_claims_param, expected_claims):
        ...
        try:
            ...
            if claims:
                ...
                for i, claim in enumerate(claims):
                    check_status = "Checkworthy" if claim.check_worthiness >= 0.5 else "Not checkworthy"
