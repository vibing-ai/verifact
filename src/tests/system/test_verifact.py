...
class VerifactTester:
    ...
    def generate_report(self):
        ...
        report_path = report_dir / f"verifact_test_report_{self.start_time.strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def check_environment(self):
        ...
        if not meets_requirement:
            self.results["issues"].append(f"Python version {python_version} does not meet minimum requirement {min_version[0]}.{min_version[1]}")
        ...
        if not env_file_exists:
            ...
            self.results["recommendations"].append("Create a .env file with required configuration (see README.md)")
        ...
        try:
            ...
            for package in ["openai", "pydantic", "fastapi", "chainlit", "supabase"]:
                try:
                    ...
                    self.results["environment_checks"][f"{package}_installed"] = {
                        "status": "PASS"
                    }
                except ImportError:
                    self.results["environment_checks"][f"{package}_installed"] = {
                    ...
                    self.results["recommendations"].append(f"Install {package} with pip install {package}")
    ...
    async def test_components(self):
        ...
        try:
            ...
            self.results["component_tests"]["verdict_writer"] = {
                ...
                "duration_seconds": duration,
                "status": "PASS" if verdict.verdict in ["TRUE", "LIKELY_TRUE", "PARTLY_TRUE", "UNCERTAIN", "LIKELY_FALSE", "FALSE", "UNVERIFIABLE"] else "FAIL"
            }
    ...
    async def test_integration(self):
        ...
        try:
            ...
            print(f"  Processed {len(test_texts)} texts with {total_claims} claims in {duration:.2f} seconds")
    ...
    async def test_api(self):
        ...
        try:
            ...
            self.results["api_tests"]["api_running"] = {
    ...
    async def test_system(self):
        ...
        try:
            ...
            for claim in claim_types:
                ...
                if results and len(results) > 0:
                    ...
                    if passed:
                        ...
                        self.results["issues"].append(f"System test for {claim['name']} failed: got {verdict}, expected one of {expected}")
                else:
                    self.results["system_tests"][claim["name"]] = {
            ...
            self.results["system_tests"]["error"] = {
                "error": str(e),
                "status": "FAIL"
            }
