...
async def test_create_and_query_claim(db_session):
    ...
    # Query the claim
    result = await db_session.execute(select(Claim).where(Claim.text.contains("Earth")))
...
async def test_create_and_query_evidence(db_session):
    ...
    # Query the evidence
    result = await db_session.execute(select(Evidence).where(Evidence.claim_id == claim.id))
...
async def test_create_and_query_verdict(db_session):
    ...
    # Query the verdict
    result = await db_session.execute(select(Verdict).where(Verdict.claim_id == claim.id))
...
async def test_api_key_management(db_session):
    ...
    # Create an API key
    api_key = ApiKey(name="Test Client", description="API key for testing", enabled=True)
    ...
    # Retrieve and verify
    result = await db_session.execute(select(ApiKey).where(ApiKey.name == "Test Client"))
