...
class TestPipelineIntegration:
    ...
    @pytest.fixture
    def patched_pipeline(self, mock_claim_detector, mock_evidence_hunter, mock_verdict_writer, mock_pipeline_config):
        """Return a pipeline with patched components for testing."""
        with patch('src.pipeline.factcheck_pipeline.ClaimDetector', return_value=mock_claim_detector), \
    ...
    @pytest.mark.asyncio
    async def test_full_pipeline_partially_true_claim(self, patched_pipeline, partially_true_claim_text):
        ...
        # Verify each component was called correctly
        patched_pipeline.claim_detector.detect_claims.assert_called_once_with(partially_true_claim_text)
        patched_pipeline.evidence_hunter.gather_evidence.assert_called_once()
        patched_pipeline.verdict_writer.generate_verdict.assert_called_once()

    @pytest.mark.asyncio
    async def test_full_pipeline_unverifiable_claim(self, patched_pipeline, unverifiable_claim_text):
        ...
        # Verify each component was called correctly
        patched_pipeline.claim_detector.detect_claims.assert_called_once_with(unverifiable_claim_text)
        patched_pipeline.evidence_hunter.gather_evidence.assert_called_once()
        patched_pipeline.verdict_writer.generate_verdict.assert_called_once()
        ...
        for verdict in results:
            ...
            for key, expected_verdict in expected_verdicts.items():
                if key in claim_text:
                    assert verdict["verdict"] == expected_verdict, f"Verdict for '{claim_text}' should be {expected_verdict}"
        
        # Verify each component was called correctly
        patched_pipeline.claim_detector.detect_claims.assert_called_once_with(mixed_claims_text)
        assert patched_pipeline.evidence_hunter.gather_evidence.call_count == 5, "EvidenceHunter should be called for each claim"
        assert patched_pipeline.verdict_writer.generate_verdict.call_count == 5, "VerdictWriter should be called for each claim"
    ...
    async def test_pipeline_no_claims(self, patched_pipeline):
        ...
        results = await patched_pipeline.process_text("This is a text with no factual claims, just opinions and descriptions.")
    ...
    async def test_pipeline_no_evidence(self, patched_pipeline, true_claim_text):
        ...
        verdict = results[0]
        assert verdict["verdict"] == "UNCERTAIN", "Verdict should be UNCERTAIN when no evidence is found"
        assert "Earth orbits" in verdict["claim"]["text"], "Claim text should be preserved"
    ...
    async def test_pipeline_streaming(self, patched_pipeline, mixed_claims_text):
        ...
        expected_snippets = ["COVID-19 pandemic", "created in a lab", "Vaccines", "New York City", "Climate change"]
        
        for snippet in expected_snippets:
            assert any(snippet in text for text in claim_texts), f"Expected to find a claim containing '{snippet}'"
    ...
    async def test_pipeline_error_recovery(self, patched_pipeline, true_claim_text):
        ...
        def fail_once(*args, **kwargs):
            ...
            return [{"text": "Evidence text", "source": "https://example.com", "credibility": 0.9, "stance": "supporting"}]
    ...
    async def test_pipeline_performance(self, patched_pipeline, mixed_claims_text):
        ...
        assert processing_time < 30.0, f"Processing time ({processing_time:.2f}s) exceeds 30s threshold"
    ...
    @pytest.mark.asyncio
    async def test_pipeline_with_real_components(self, mock_openai_client, mock_web_search, env_setup):
        """Test the pipeline with actual component implementations (not mocks)."""
        with patch('src.agents.claim_detector.openai.OpenAI', return_value=mock_openai_client), \
    ...
    def test_data_flow_integrity(self, patched_pipeline, true_claim_text):
        ...
        verdict_writer_input_claim = patched_pipeline.verdict_writer.generate_verdict.call_args[0][0]
        verdict_writer_input_evidence = patched_pipeline.verdict_writer.generate_verdict.call_args[0][1]
        
        # Verify data integrity
        assert evidence_hunter_input == claim_detector_output[0], "Claim passed to EvidenceHunter should match ClaimDetector output"
        assert verdict_writer_input_claim == claim_detector_output[0], "Claim passed to VerdictWriter should match ClaimDetector output"
        assert verdict_writer_input_evidence == evidence_hunter_output, "Evidence passed to VerdictWriter should match EvidenceHunter output"

    @pytest.mark.asyncio
    async def test_pipeline_configuration(self, mock_claim_detector, mock_evidence_hunter, mock_verdict_writer):
        ...
        with patch('src.pipeline.factcheck_pipeline.ClaimDetector', return_value=mock_claim_detector), \
            ...
            assert mock_evidence_hunter.gather_evidence.call_args[1].get('max_results', None) == 1, \
    ...
    for test in test_inputs:
        ...
        try:
            ...
            for i, claim in enumerate(checkworthy_claims):
                ...
                claim_result["steps"]["verdict_generation"] = {
                ...
                claim_result["verdict"] = {
                    ...
                    "explanation": verdict.explanation[:300] + ("..." if len(verdict.explanation) > 300 else ""),
