...
async def test_pipeline_e2e_controversial_claim(pipeline):
    ...
    assert verdict.alternative_perspectives is not None, "Should include alternative perspectives for controversial claims"
...
async def test_pipeline_streaming(pipeline):
    ...
    verdicts = next(data for event_type, data in stream_results 
                   if event_type == PipelineEvent.COMPLETED)["verdicts"]
