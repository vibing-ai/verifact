...
class Verdict(BaseModel):
    ...
    generated_at: datetime = Field(
        default_factory=datetime.now, description="When the verdict was generated"
    )
...
class PipelineConfig(BaseModel):
    ...
    claim_detection_threshold: float = Field(
        0.5, ge=0.0, le=1.0, description="Threshold for claim detection confidence"
    )
    max_claims: int = Field(10, ge=1, le=100, description="Maximum number of claims to process")
    max_evidence_per_claim: int = Field(
        5, ge=1, le=50, description="Maximum pieces of evidence per claim"
    )
    ...
    min_check_worthiness: float = Field(
        0.5, ge=0.0, le=1.0, description="Minimum threshold for claim check-worthiness"
    )
    explanation_detail: Literal["brief", "standard", "detailed"] = Field(
        "standard", description="Level of detail for verdict explanations"
    )
...
class FactcheckRequest(BaseModel):
    ...
    def validate_options(cls, v):
        ...
        for list_field in ["domains", "blocked_domains", "claim_categories"]:
            if (
                list_field in v
                and v[list_field] is not None
                and not isinstance(v[list_field], list)
            ):
