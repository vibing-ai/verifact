...
from src.verifact_agents.interfaces import ClaimDetector, EvidenceHunter, VerdictWriter
...
class FactcheckPipeline:
    ...
    def __init__(
        ...
        logger.info(
            "FactcheckPipeline initialized with min_check_worthiness=%f, max_claims=%d",
    ...
    async def _detect_claims(self, text: str) -> list[Claim]:
        ...
        claims = await self.claim_detector.detect_claims(
            text, min_check_worthiness=self.min_check_worthiness, max_claims=self.max_claims
        )
        logger.info(
            "Detected %d claims with check-worthiness >= %f", len(claims), self.min_check_worthiness
        )
        return claims
    ...
    async def _gather_evidence_for_claims(
        ...
        """
        logger.info(
            "Gathering evidence for %d claims with parallelism %d", len(claims), self.parallelism
        )
