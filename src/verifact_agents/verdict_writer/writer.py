...
class VerdictWriter(IVerdictWriter):
    ...
    def _rank_evidence(self, evidence_list: list[Evidence]) -> list[dict]:
        ...
        for evidence in evidence_list:
            ...
            ranked_evidence.append({"evidence": evidence, "quality_score": quality_score})
    ...
    def _calculate_confidence_score(self, claim: str, evidence_list: list[Evidence]) -> float:
        ...
        if top_evidence:
            average_quality = sum(item["quality_score"] for item in top_evidence) / len(
                top_evidence
            )
        else:
            average_quality = 0.3
        ...
        combined_score = (
            (quantity_score * 0.3) + (average_quality * 0.5) + (consistency_score * 0.2)
        )
    ...
    async def generate_verdict(
        ...
        try:
            ...
            logger.info(
                f"Generated verdict: {verdict.verdict} with confidence {verdict.confidence:.2f}"
            )
