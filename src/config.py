...
class ModelSelectionConfig(BaseModel):
    ...
    evidence_hunter_model: str = Field("google/gemma-3-27b-it:free", description="Evidence hunter model")
    verdict_writer_model: str = Field("deepseek/deepseek-chat:free", description="Verdict writer model")
...
class Settings(BaseSettings):
    ...
    def build_config(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result["ui"].setdefault("auth_enabled", os.getenv("CHAINLIT_AUTH_ENABLED", "false").lower() == "true")
        ...
        result["models"].setdefault("default_model", os.getenv("DEFAULT_MODEL", "meta-llama/llama-3.3-8b-instruct:free"))
        result["models"].setdefault("claim_detector_model", os.getenv("CLAIM_DETECTOR_MODEL", "qwen/qwen3-8b:free"))
        result["models"].setdefault("evidence_hunter_model", os.getenv("EVIDENCE_HUNTER_MODEL", "google/gemma-3-27b-it:free"))
        result["models"].setdefault("verdict_writer_model", os.getenv("VERDICT_WRITER_MODEL", "deepseek/deepseek-chat:free"))
        result["models"].setdefault("embedding_model", os.getenv("EMBEDDING_MODEL", "text-embedding-3-small"))
        result["models"].setdefault("enable_caching", os.getenv("ENABLE_MODEL_CACHING", "true").lower() == "true")
        ...
        result["model_params"].setdefault("request_timeout", os.getenv("MODEL_REQUEST_TIMEOUT", "120"))
        ...
        result["logging"].setdefault("daily_rotation", os.getenv("LOG_DAILY_ROTATION", "false").lower() == "true")
        
        # Construct search config
        result.setdefault("search", {})
        result["search"].setdefault("use_serper", os.getenv("USE_SERPER", "false").lower() == "true")
